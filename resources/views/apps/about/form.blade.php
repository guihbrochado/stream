                        ...
                        <div class="col-xl-9 col-lg-8">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    @endif
                                    <form action="{{ $action === 'create' ? route('about.store') : route('about.update', $about->id) }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                        @if($action === 'edit')
                                        @method('PUT')
                                        @endif
                                        <div class="row">
                                            <div class="form-group col-md-6">
                                                ...
                                                <input type="text" class="form-control" id="title" name="title" placeholder="Título" required value="{{ old('title', $about->title) }}">
                                            </div>
                                            <div class="form-group col-md-6">
                                                <label for="contact_email">E-mail de Contato:</label>
                                                <input type="email" class="form-control" id="contact_email" name="contact_email" placeholder="E-mail de Contato" required value="{{ old('contact_email', $about->contact_email) }}">
                                            </div>
                                            <div class="form-group col-md-12">
                                                <label for="description">Descrição:</label>
                                                <textarea class="form-control" id="description" name="description" rows="4" placeholder="Descrição detalhada sobre a empresa" required>{{ old('description', $about->description) }}</textarea>
                                            </div>
                                            <div class="form-group col-md-6">
                                                <label for="cover_image">Capa:</label>
                                                <input type="file" class="form-control" id="cover_image" name="cover_image">
                                                @if($about->cover_image)
                                                <img src="{{ asset($about->cover_image) }}" alt="Capa" style="max-width: 100px; max-height: 100px;">
                                                @endif
                                            </div>
                                        </div>
                                        <button type="submit" class="btn btn-primary mt-3">{{ $action === 'create' ? 'Salvar' : 'Atualizar' }}</button>
                                    </form>
