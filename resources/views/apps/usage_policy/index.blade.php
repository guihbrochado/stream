                        ...
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <div class="header-title">
                                    <h4 class="card-title">Políticas de Uso</h4>
                                </div>
                                <!-- Botão para Adicionar Política -->
                                <div>
                                    <a href="{{ route('usage_policy.create') }}" class="btn btn-primary">Adicionar Política</a>
                                </div>
                            </div>
                            <div class="card-body">
                                <div class="table-responsive border rounded py-4">
                                    @foreach ($data->groupBy('usage_policy_category_id') as $category_id => $policies)
                                    <div class="mb-4">
                                        <h5 class="category-title text-center mb-3">{{ $policies->first()->title }}</h5>
                                        <table id="datatable" class="table" data-toggle="data-table">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-center">{{ $policy->question }}</td>
                                                    <td class="text-center">{{ Str::limit($policy->answer, 100) }}</td>
                                                    <td class="text-center">
                                                        <a href="{{ route('usage_policy.edit', $policy->id) }}"> <i class="fas fa-edit me-2 lh-lg"></i> </a>
                                                        <form action="{{ route('usage_policy.destroy', $policy->id) }}" method="POST" style="display:inline;">
                                                            @csrf
                                                            @method('DELETE')
